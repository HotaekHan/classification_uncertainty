...
model_urls = {
    ...
    'resnet101d': 'https://download.pytorch.org/models/resnet101-5d3b4d8f.pth',
...
class ResNetD(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.conv1 = nn.Sequential(
            ...
            norm_layer(32),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=32, out_channels=self.inplanes, kernel_size=3, stride=1, padding=1, bias=False)
        )
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            if stride == 1:
                ...
                pool = nn.AvgPool2d(kernel_size=2, stride=stride, ceil_mode=True, count_include_pad=False)
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    model = ResNetD(block, layers, **kwargs)
...
def resnet101d(pretrained=False, progress=True, **kwargs):
...
def resnet152d(pretrained=False, progress=True, **kwargs):
...
if __name__ == '__main__':
    net = resnet50d(pretrained=False, progress=False, num_classes=100)
