...
def conv2d_BN(ni, nf, stride, activation="leaky_relu", kernel_size=3, activation_param=1e-2, groups=1):
    if activation == 'leaky_relu':
        act = nn.LeakyReLU(negative_slope=activation_param, inplace=True)
    elif activation == 'identity':
        act = nn.Identity()
    elif activation == 'relu':
        act = nn.ReLU(inplace=True)
    else:
        raise ValueError('Unsupported activation fn: ' + str(activation))
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, use_se=True, anti_alias_layer=None):
        ...
        if stride == 1:
            self.conv1 = conv2d_BN(inplanes, planes, stride=1, activation_param=1e-3)
        else:
            if anti_alias_layer is None:
                self.conv1 = conv2d_BN(inplanes, planes, stride=2, activation_param=1e-3)
            else:
                self.conv1 = nn.Sequential(conv2d_BN(inplanes, planes, stride=1, activation_param=1e-3),
                                           anti_alias_layer(channels=planes, filt_size=3, stride=2))

        self.conv2 = conv2d_BN(planes, planes, stride=1, activation="identity")
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, use_se=True, anti_alias_layer=None):
        ...
        self.conv1 = conv2d_BN(inplanes, planes, kernel_size=1, stride=1, activation="leaky_relu",
                                activation_param=1e-3)
        if stride == 1:
            self.conv2 = conv2d_BN(planes, planes, kernel_size=3, stride=1, activation="leaky_relu",
                                    activation_param=1e-3)
        else:
            if anti_alias_layer is None:
                self.conv2 = conv2d_BN(planes, planes, kernel_size=3, stride=2, activation="leaky_relu",
                                        activation_param=1e-3)
            else:
                self.conv2 = nn.Sequential(conv2d_BN(planes, planes, kernel_size=3, stride=1,
...
class TResNet(nn.Module):
    ...
    def __init__(self, layers, in_chans=3, num_classes=1000, width_factor=1.0, remove_aa_jit=False):
        ...
        conv1 = conv2d_BN(in_chans * 16, self.planes, stride=1, kernel_size=3)
