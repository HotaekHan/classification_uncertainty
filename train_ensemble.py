...
def iterate(epoch, phase):
    ...
    with torch.set_grad_enabled(is_train):
        ...
        if is_train is True:
            ...
            filename = 'latest_' + str(iter_ensemble) + '.pth'
            torch.save(state, os.path.join(config['exp']['path'], filename))
        else:
            ...
            if is_saved is True:
                print('[Valid] Saving..')
                filename = 'best_' + str(iter_ensemble) + '.pth'
                # torch.save(state, config['model']['exp_path'] + '/ckpt-' + str(epoch) + '.pth')
                torch.save(state, os.path.join(config['exp']['path'], filename))


if __name__ == '__main__':
    num_ensembles = config['params']['num_ensembles']
    ...
    seeds = np.random.randint(low=1, high=1000, size=num_ensembles)

    for iter_ensemble in range(num_ensembles):
        print('Do ensemble for ' + str(iter_ensemble+1))
        start_epoch = 0  # start from epoch 0 or last epoch

        '''set random seed'''
        random.seed(seeds[iter_ensemble])
        np.random.seed(seeds[iter_ensemble])
        torch.manual_seed(seeds[iter_ensemble])
        os.environ["PYTHONHASHSEED"] = str(seeds[iter_ensemble])
        ...
        '''tensorboard'''
        summary_writer = SummaryWriter(os.path.join(config['exp']['path'], 'log_' + str(iter_ensemble)))
